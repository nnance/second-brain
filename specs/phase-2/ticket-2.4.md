# Ticket 2.4: Implement vault_read Tool

## Description
Create the `vault_read` tool that the AI agent uses to read existing notes from the Obsidian vault. This enables the agent to check for related notes, read previous context, or verify stored content. The tool is a pure function that reads and returns file content.
...
- Only allow reading from known vault subfolders
