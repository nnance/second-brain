# Ticket 2.7: Implement send_message Tool

## Description
Create the `send_message` tool that the AI agent uses to reply to users via iMessage. This enables the agent to send confirmations, ask clarifying questions, and provide feedback. The tool wraps the imessage-kit library to send messages.

## Acceptance Criteria
- [ ] Tool exists at `src/tools/send-message.ts`
- [ ] Accepts message text and recipient (from original message context)
...
### Message Context
The recipient is extracted from the original incoming message. When the agent runner invokes this tool, it provides the recipient from the conversation context.
